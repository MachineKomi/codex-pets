# Mild8 Neko generation prompts

Generation mode: built-in OpenAI image generator, approved by user; exact model identifier not exposed.
...
Create one horizontal four-cardinal anchor strip for Codex pet `mild8-neko`.
...
Repair one cardinal anchor for Codex pet `mild8-neko`: `000` means looking up.
...
Repair one cardinal anchor for Codex pet `mild8-neko`: `090` means looking right.
...
Repair one cardinal anchor for Codex pet `mild8-neko`: `180` means looking down.
...
Repair one cardinal anchor for Codex pet `mild8-neko`: `270` means looking left.
...
Create Codex pet row `failed` for `mild8-neko`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Original adult woman aged 28, mature womanly proportions in compact full-body anime sprite form. Messy shoulder-length golden blonde hair with dark roots, golden cat ears, deep orange-brown fake tan, tired half-lidded brown eyes, smudged black gyaru eyeliner and pale lip gloss. Stained charcoal cropped tank top, loose worn denim shorts, exposed navel with silver belly button piercing, grubby white crew socks with sock-covered feet and no shoes, blonde cat tail. Slouched lazy posture and grungy slob comedy. Small plain cigarette held between lips, no smoke clouds. Distinct original silhouette and outfit, no existing character replica, no logos or readable cigarette branding. Adult, clothed, non-explicit.. Preserve silhouette, face, palette, material, proportions, markings, and props.
...
Create Codex pet row `idle` for `mild8-neko`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `jumping` for `mild8-neko`: exactly 5 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex v2 pet look row 10 for `mild8-neko` as exactly 8 full-body frames in this order: 180, 202.5, 225, 247.5, 270, 292.5, 315, 337.5.
...
Create Codex v2 pet look row 9 for `mild8-neko` as exactly 8 full-body frames in this order: 000, 022.5, 045, 067.5, 090, 112.5, 135, 157.5.
...
Create Codex pet row `review` for `mild8-neko`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running-left` for `mild8-neko`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running-right` for `mild8-neko`: exactly 8 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `running` for `mild8-neko`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `waiting` for `mild8-neko`: exactly 6 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create Codex pet row `waving` for `mild8-neko`: exactly 4 full-body frames in one horizontal strip on flat pure user-selected #00FF00.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `failed`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `idle`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `jumping`.
...
Create one horizontal look-direction strip for Codex pet `mild8-neko`, atlas row 10.
...
Create one horizontal look-direction strip for Codex pet `mild8-neko`, atlas row 9.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `review`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `running-right`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `running`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `waiting`.
...
Create one horizontal animation strip for Codex pet `mild8-neko`, state `waving`.
